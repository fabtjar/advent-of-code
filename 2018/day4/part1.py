...
for guard, sleep in guards.items():
    if sleep['total_sleep'] >= most_sleep:
        most_sleep = sleep['total_sleep']
        sleep_guard = guard
print('Guard with most sleep: #{}'.format(sleep_guard))

most_sleep = 0
for time, sleep in guards[sleep_guard]['sleep'].items():
    if sleep >= most_sleep:
        most_sleep = sleep
        sleep_time = time
print('Time with most sleep: {}'.format(sleep_time))
